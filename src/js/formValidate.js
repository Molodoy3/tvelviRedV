...
//можно указать минимальную и максимальную длину для полей:
// data-req-min
// data-req-max

//можно указать минимальную и максимальную длину для чисел:
//data-req-num - проверка на число
// data-req-num-min
// data-req-num-max
...
if (forms.length) {
    ...
    for (let index = 0; index < forms.length; index++) {
        ...
        if (url) {
            //? тут надо указывать не form, а forms[index] всегда, иначе будет обработка только последней формы
            forms[index].addEventListener('submit', (e) => {
                //e.preventDefault();
                let errors = formValidate(forms[index]);
                if (errors === 0) {

                    /* //!Отправка формы
                    const url = urls[forms[index].name];
                    ...
                    const errorElements = forms[index].querySelectorAll('.error-element');
                    ...
                    //?Ошибка
                    e.preventDefault();
                    forms[index].classList.add('error');
                }
            });
    ...
    function formValidate(form) {
        ...
        if (formReq.length) {
            for (let index = 0; index < formReq.length; index++) {
                ...
                //Проверка на число
                else if (input.hasAttribute("data-req-num")) {
                ...
                //Дополнительные проверка на минимальную и максимальную числа
                const minNum = input.dataset.reqNumMin;
                if (input.dataset.reqNumMin && input.value < minNum) {
                    formAddError(input);
                    errors++;
                    isMessageOutput ? null : setTextError(input);
                }
                const maxNum = input.dataset.reqNumMax;
                if (input.dataset.reqNumMax && input.value > maxNum) {
                    formAddError(input);
                    errors++;
                    isMessageOutput ? null : setTextError(input);
                }
